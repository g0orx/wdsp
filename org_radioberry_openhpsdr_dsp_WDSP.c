#include "org_radioberry_openhpsdr_dsp_WDSP.h"
...
(JNIEnv* env, jobject obj, jint channel, jint ss, jint LO, jdoubleArray i, jdoubleArray q) {
	jdouble* fi = (*env)->GetDoubleArrayElements(env, i, 0);
	jdouble* fq = (*env)->GetDoubleArrayElements(env, q, 0);
...
(JNIEnv* env, jobject obj, jint disp, jint n_pixout, jint n_fft, jint typ, jintArray flp, jint sz, jint bf_sz, jint win_type, jdouble pi, jint ovrlp, jint clp, jint fscLin, jint fscHin, jint n_pix, jint n_stch, jint calset, jdouble fmin, jdouble fmax, jint max_w) {
	jint* flpptr = (*env)->GetIntArrayElements(env, flp, 0);
	SetAnalyzer(disp, n_pixout, n_fft, typ, flpptr, sz, bf_sz, win_type, pi, ovrlp, clp, fscLin, fscHin, n_pix, n_stch, calset, fmin, fmax, max_w);
...
(JNIEnv* env, jobject obj, jint disp, jfloatArray pix, jintArray flag) {
	...
	GetPixels(disp, 0, pixptr, flagptr);
